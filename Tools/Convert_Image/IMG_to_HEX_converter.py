...
if img.width != int(W) or img.height != int(H):
    print(f"Image size ({img.width}x{img.height}) does not match input ({W}x{H}), resizing...")
    img = img.resize((int(W), int(H)), Image.LANCZOS)
    print(f"Image size new: {img.width}x{img.height}")

f = open("picFile.txt", "w")  # Nên dùng "w" để ghi đè file cũ
...
for y in range(0, img.height):
    ...
    for x in range(0, img.width):
        ...
        # for right endianness, so ST7735_DrawImage would work
